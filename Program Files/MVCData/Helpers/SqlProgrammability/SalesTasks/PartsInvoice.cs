...
namespace MVCData.Helpers.SqlProgrammability.SalesTasks
{
    public class PartsInvoice
    {
        ...
        {
            this.GetCommoditiesInWarehouses(false);
            this.GetCommoditiesInWarehouses(true);
        ...
        /// </summary>
        private void GetCommoditiesInWarehouses(bool withCommoditiesInGoodsReceipts)
        {
            string queryString = " @LocationID int, @EntryDate DateTime, @SearchText nvarchar(60) " + (withCommoditiesInGoodsReceipts? "": " , @SalesInvoiceID int, @StockTransferID int, @StockAdjustID int ") + "\r\n";
            ...
            queryString = queryString + "       INSERT INTO @Commodities SELECT CommodityID, Code, Name, GrossPrice, CommodityTypeID, CommodityCategoryID FROM Commodities WHERE CommodityTypeID IN (" + (int)GlobalEnums.CommodityTypeID.Parts + ", " + (int)GlobalEnums.CommodityTypeID.Consumables + (withCommoditiesInGoodsReceipts ? ", " + (int)GlobalEnums.CommodityTypeID.Vehicles : "") + ") AND (Code LIKE '%' + @SearchText + '%' OR Name LIKE '%' + @SearchText + '%') " + "\r\n";

            queryString = queryString + "       IF (@@ROWCOUNT > 0) " + "\r\n";
            queryString = queryString + "           " + this.GetCommoditiesInWarehousesBuildSQL(withCommoditiesInGoodsReceipts) + "\r\n";
            queryString = queryString + "       ELSE " + "\r\n";
            queryString = queryString + "           " + this.GetCommoditiesInWarehousesRETURNNothing() + "\r\n";

            this.totalBikePortalsEntities.CreateStoredProcedure(withCommoditiesInGoodsReceipts ? "GetCommoditiesAvailables" : "GetCommoditiesInWarehouses", queryString); //GetCommoditiesAvailables is used in TransferOrder (Need to get both Vehicles and Parts/ Consumables on the same view)
        }

        private string GetCommoditiesInWarehousesGETAvailable(bool withCommoditiesInGoodsReceipts)
        {
            SqlProgrammability.StockTasks.Inventories inventories = new StockTasks.Inventories(this.totalBikePortalsEntities);


            string queryString = "                      DECLARE @WarehouseIDList varchar(35)        DECLARE @CommodityIDList varchar(3999) " + "\r\n";
        ...
        private string GetCommoditiesInWarehousesBuildSQL(bool withCommoditiesInGoodsReceipts)
        {
            string queryString = "                  BEGIN " + "\r\n";

            queryString = queryString + "               " + this.GetCommoditiesInWarehousesGETAvailable(withCommoditiesInGoodsReceipts) + "\r\n";
